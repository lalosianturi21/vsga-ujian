...
function list_wisata()
{
    ...
    while ($row = mysqli_fetch_array($query)) {
        $opt .= "<option value=\"" . $row['idwisata'] . "\">" . $row['nama'] . " " . $row['harga'] .  "</option>";
    }
    return $opt;
}
...
{
    include('conn.php');
    $query = mysqli_query($con, "SELECT * FROM services ORDER BY service_name ASC");
    $opt = "";
    while ($row = mysqli_fetch_array($query)) {
        $opt .= "<option value=\"" . $row['service_id'] . "\">" . $row['service_name'] . " " . $row['service_price'] . "</option>";
    }
    return $opt;
}
